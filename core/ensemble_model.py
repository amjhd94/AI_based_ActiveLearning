    ...
    def __init__(self, model_class, train, val_pct=0, N=2, lr=0.001, epochs=3000, 
        ...
        for i in range(self.N):
            ...
            Model.compile(optimizer=keras.optimizers.Adam(lr=self.lr),
                          ...
                          metrics=['mse'])
            Model.fit(train[0], train[1], validation_split=val_pct, epochs=self.epochs, batch_size=self.batch_size, verbose=self.fit_verbose)# Once you are done training, just save the model definition and it's learned weights
